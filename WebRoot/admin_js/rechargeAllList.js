	...
	var C = function () {
		var D = function () {
    	    ...
    	    function pageselectCallbackConsume(pageNo) {
		    	var url = "/user/ConsumeListByDeltaAjaxPage.action?pageNo="+pageNo+"&pageSize=20&startDate="+Q.beginTime+"&endDate="+Q.endTime;
		            ...
		            success: function (data) {
		            	...
		            	if(resultCount>0){
			            	...
			            	for(var i=0;i<data.length;i++){
			            		$("<tr><td>"+data[i].userId+"</td><td>￥"+data[i].money+"</td><td>网银支付</td><td>"+data[i].date+"</td></tr>").appendTo(G);
			            	}
			            	$("#pageLoading").hide();
		            	}else{
       	    ...
       	    function pageselectCallbackConsume2(pageNo) {
		    	var url = "/user/ConsumeListByDeltaAjaxPage.action?pageNo="+pageNo+"&pageSize=20&userId="+$("#userId").val()+"&startDate="+Q.beginTime+"&endDate="+Q.endTime;
		    	$(".pageULEx").prepend("<li class=\"total_page\" id=\"pageLoading\"><img src=\"/Images/loding.gif\" /></li>");
		        $.ajax({
		            url: url,
		            type: "post",
			...
			var R = function () {
				...
				$.ajax({
					url:"/user/ConsumeListByDeltaAjaxPageResultCount.action?pageNo="+pageNo+"&userId="+$("#userId").val()+"&startDate="+Q.beginTime+"&endDate="+Q.endTime,
					type:"get",
					data:"String",
					success:function(data){
