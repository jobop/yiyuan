...
<input type="hidden" id="proImg" src="${productInfo.headImage  }"/>
<input type="hidden" id="surplus" value="${productInfo.productPrice - productInfo.spellbuyCount}"/>
	    ...
	    <div class="Pro_Details">
		    ...
		    <div class="Pro_Detright">
			        ...
			        <div class="Progress-bar">
				        <p title="已完成<fmt:formatNumber type="number" value="${(productInfo.spellbuyCount/productInfo.productPrice)*100 } " maxFractionDigits="2"/> %"><span style="width:<fmt:formatNumber type="number" value="${(productInfo.spellbuyCount/productInfo.productPrice)*100 } " maxFractionDigits="2"/>%;"></span></p>
				        <ul class="Pro-bar-li">
					        ...
					        <li class="P-bar03"><em id="CodeLift" singlePrice="${productInfo.singlePrice }">${productInfo.productPrice - productInfo.spellbuyCount}</em>剩余人次</li>
...
<!--			        <a id="btnGoToPost" class="gray02" href="javascript:;">本商品已有<span class="Fb blue">33</span>个幸运者晒单   已有<span class="Fb blue">2719</span>条晒单评论</a>-->
			        </p><div class="Pro_number" id="divNumber">
				        我要<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %> <a class="num_del num_ban" href="javascript:;">-</a><input type="text" class="num_dig" maxlength="7" value="${productInfo.singlePrice }" /><a class="num_add" href="javascript:;">+</a> 人次 <span class="gray03" id="chance"><font color="ff6600">购买人次越多获得几率越大哦！</font></span>
			        </div><div class="Det_button" id="divBuy">
				        <a class="Det_Shopbut" href="javascript:;">立即<%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%></a><a class="Det_Cart" href="javascript:;"><i></i>加入购物车</a>
			        </div>
			        <div class="Security">
				        <ul>
			        ...
			        <div class="Pro_Record">
				        		...
				        		<c:when test="${fn:length(ParticipateJSONList)!=0}">
				        			<ul>
							            <s:iterator var="ParticipateJSON" value="ParticipateJSONList">
							            	<li><a target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${ParticipateJSON.userId }.html" rel="nofollow"><img style="width:20px;height:20px;" border="0" alt="" src="${ParticipateJSON.userFace }" /></a><a class="blue" target="_blank" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/u/${ParticipateJSON.userId }.html">${ParticipateJSON.userName }</a>(${ParticipateJSON.ip_location }) ${ParticipateJSON.buyDate } <%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>了<em class="Fb gray01">${ParticipateJSON.buyCount }</em>人次</li>
							            </s:iterator>
					                </ul>
							        <p style=""><a class="gray01" href="javascript:;" id="btnUserBuyMore">查看更多</a></p>
				        		</c:when>
				        		<c:otherwise>
				        			<div class="NewRecord_msg"><s></s><span class="NewRecord_msgtxt"><em>居然还没有人参与？! </em>机不可失！ 大奖正在向您挥手！</span></div>
				        		</c:otherwise>
				        	</c:choose>
				        </div>
					    ...
					    <div style="display:none;" class="Newest_Con">
					        ...
					        <p class="MsgIntro1"><%=ApplicationListenerImpl.sysConfigureJson.getSaitName()%>以“快乐<%=ApplicationListenerImpl.sysConfigureJson.getShortName() %>，惊喜无限”为宗旨，力求打造一个100%公平公正、100%正品保障、寄娱乐与购物一体化的新型购物网站。<a target="_blank" class="blue" href="<%=ApplicationListenerImpl.sysConfigureJson.getWwwUrl()%>/help/whatPaigou.html">查看详情&gt;&gt;</a></p>
					    </div>
			        </div>
			    </div>
		    </div>
